...
export function ProductionUserRow({ userData, dailyLimit }) {
  ...
  return (
    <div className={exceeds ? 'bg-red-500/5' : ''}>
      <div className="grid grid-cols-6 gap-2 p-2 items-center text-sm">
        <div className="w-8">
          <Button
            variant="ghost"
            size="sm"
            ...
            {userData.entries.map((entry, i) => (
              <div key={i} className="flex items-center justify-between text-sm py-1 px-2 bg-background/50 rounded">
                <div className="flex items-center gap-3">
                  ...
                  {entry.batch_id && (
                    <span className="font-mono text-xs text-muted-foreground">Batch: {entry.batch_id.slice(-8)}</span>
                  )}
                </div>
                <div className="flex items-center gap-4 text-muted-foreground">
                  <span>{entry.items_processed} frames</span>
